// Script to execute SQL directly through the MCP server
const fetch = require('node-fetch');

async function executeSql(sql) {
  try {
    const response = await fetch('http://localhost:4567/query-table', {
      method: 'POST',
      headers: {
    ...
    // If we get here without an error, we can use the query-table endpoint
    return { success: true, data: result };
  } catch (error) {
    console.error('Error executing SQL through query-table:', error);
    return { success: false, error: error.message };
  }
}

// SQL to create the messaging tables
